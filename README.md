...
⚠️ aarch64 releases are currently unavailable due to a build issue outside our control. We need to run `go build` to compile Rclone, which recently introduced a new C/C++ dependency. `go build` tries to force the gold linker even though it is unavailable in Flathub's CI. When this issue is resolved or a workaround is provided allowing us to select a different linker, we can re-enable aarch64 support.
